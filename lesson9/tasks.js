...
let ulLi = document.body.appendChild(ul);
...
for (const elements of coursesAndDurationArray) {
    ...
    elementsDiv.innerText = `${elements.title} ${elements.monthDuration}`;
...
//     Завдання робити через цикли.
for (const elements of coursesAndDurationArray1) {
...
for (const element of simpsons) {
    ...
    div.innerText = `${element.name} ${element.surname} ${element.age} ${element.info} ${element.photo}`;
    document.body.appendChild(div);
}
// =========================
//     Цикл в циклі
...
for (const element of coursesArray) {
    ...
    for (const elements of element.modules) {
        let module = document.createElement('li');
        module.innerText = elements;
        modules.append(module);
}
    course.append(title);
    course.append(monthDuration);
    course.append(hourDuration);
